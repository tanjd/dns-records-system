...
class ClustersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create cluster" do
    assert_difference('Cluster.count') do
      post clusters_url, params: { cluster: { name: @cluster.name, subdomain: @cluster.subdomain } }
    end
  ...
  test "should update cluster" do
    patch cluster_url(@cluster), params: { cluster: { name: @cluster.name, subdomain: @cluster.subdomain } }
